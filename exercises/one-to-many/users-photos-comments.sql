...
-- create a table of photos upload by users, with and ID autogenerated that is also the Primary Key
-- related to users table on many-to-one by user_id reference
-- every time a user is deleted, its photos are deleted too.
CREATE TABLE photos (
  id SERIAL PRIMARY KEY,
  url VARCHAR(200),
...
-- every time a user or a photo is deleted, its comments related are deleted too.
